    ...
    while (true) {
      try {
        ...
        if (!isTossConfirmed && tossWinner && tossChoice) {
          ...
          console.log(`
============================================================
🏏 TOSS DETECTED — Game #${targetMatch.matchNo}: ${targetMatch.home} vs ${targetMatch.away}
   ${tossWinner} won the toss and chose to ${tossChoice.toUpperCase()}.
   Batting First : ${battingTeamFull}
   Chasing       : ${chasingTeam}
   ✅ 1st Innings predictions are now OPEN.
============================================================`);
        ...
        if (isTossConfirmed && score && score.length > 0) {
          ...
          if (!firstInningsResolved) {
            ...
            if (!isFirstInningsLocked && s1 && s1.o >= 3.0 && graceElapsed >= 15 * 60 * 1000) {
              ...
              // --- DISCORD: 1st Innings Predictions Paused ---
              await sendDiscordNotification(
                `🔒 **Predictions are now CLOSED for the 1st Innings** (${s1.o} overs bowled)`,
                {
                  title: `${targetMatch.home} vs ${targetMatch.away} — 1st Innings`,
                  description: `Predictions locked after **${s1.o} overs**.\n${battingTeamFull}: **${s1.r}/${s1.w}**\nNo more entries accepted for this innings.`,
                  color: 0xf44336,
                  footer: { text: `Match: ${targetMatch.titleStr}` },
                  timestamp: new Date().toISOString()
                }
...
============================================================`);
              ...
              await sendDiscordNotification(
                `📊 **1st Innings complete!** ${battingTeamFull} scored **${s1.r}/${s1.w}** in ${s1.o} overs.`,
                {
                  title: `${targetMatch.home} vs ${targetMatch.away} — 1st Innings Results`,
                  description: `**Final Score:** ${battingTeamFull} ${s1.r}/${s1.w} (${s1.o} ov)\n` +
                    `**${i1Entries.length}** prediction(s) scored\n` +
                    `🥇 Highest: **${i1Top ? `${i1Top.name} — ${i1Top.points} pts` : 'No submissions'}**`,
                  color: 0x42a5f5,
                  footer: { text: `Match: ${targetMatch.titleStr}` },
                  timestamp: new Date().toISOString()
          ...
          // 3. SECOND INNINGS
            ...
            if (!isSecondInningsLocked && s2.o >= 3.0) {
              ...
              // --- DISCORD: 2nd Innings Predictions Paused ---
              await sendDiscordNotification(
                `🔒 **Predictions are now CLOSED for the 2nd Innings** (${s2.o} overs bowled)`,
                {
                  title: `${targetMatch.home} vs ${targetMatch.away} — 2nd Innings`,
                  description: `Predictions locked after **${s2.o} overs**.\n${chasingTeam}: **${s2.r}/${s2.w}** chasing ${s1 ? s1.r + 1 : '---'}\nNo more entries accepted for this innings.`,
                  color: 0xf44336,
                  footer: { text: `Match: ${targetMatch.titleStr}` },
                  timestamp: new Date().toISOString()
                }
            ...
            if (s2.o >= 19.6 || s2.w >= 10 || matchWinner || (s1 && s2.r > s1.r)) {
              ...
              let resultStr = '';
              if (isChaserWinner) {
                const wicketsInHand = 10 - s2.w;
                resultStr = `${matchWinner} won by ${wicketsInHand} wicket${wicketsInHand !== 1 ? 's' : ''} (chased in ${s2.o} ov)`;
              } else if (matchWinner) {
                const runsMargin = s1 && s2 ? s1.r - s2.r : '?';
                resultStr = `${matchWinner} won by ${runsMargin} run${runsMargin !== 1 ? 's' : ''}`;
              } else {
                resultStr = 'Result unclear / match tied';
              }
              console.log(`
============================================================
🏆 2ND INNINGS COMPLETE — Game #${targetMatch.matchNo}: ${targetMatch.home} vs ${targetMatch.away}
   Result: ${resultStr}
   ${battingTeamFull}: ${s1 ? `${s1.r}/${s1.w} (${s1.o} ov)` : '---'}
   ${chasingTeam}  : ${s2.r}/${s2.w} (${s2.o} ov)
   ⏳ Resolving 2nd innings points...
============================================================`);
              ...
              // Log 2nd innings highest scorer
              const i2Entries = Object.values(preds).filter(p => p.name);
              i2Entries.sort((a, b) => (b.points || 0) - (a.points || 0));
              const i2Top = i2Entries[0];
              console.log(`
📊 2ND INNINGS RESOLVED — Game #${targetMatch.matchNo}
   Participants : ${i2Entries.length} prediction(s) scored
   🥇 Highest  : ${i2Top ? `${i2Top.name} — ${i2Top.points} pts` : 'No submissions'}`);

              // --- DISCORD: Match Result ---
              await sendDiscordNotification(
                `🏆 **Match Result: ${resultStr}**`,
                {
                  title: `${targetMatch.home} vs ${targetMatch.away} — Final Result`,
                  description: `**${battingTeamFull}:** ${s1 ? `${s1.r}/${s1.w} (${s1.o} ov)` : '---'}\n` +
                    `**${chasingTeam}:** ${s2.r}/${s2.w} (${s2.o} ov)\n\n` +
                    `🥇 2nd Innings top scorer: **${i2Top ? `${i2Top.name} — ${i2Top.points} pts` : 'No submissions'}**`,
                  color: 0x7c4dff,
                  footer: { text: `Match: ${targetMatch.titleStr}` },
                  timestamp: new Date().toISOString()
                }
              ...
              console.log("[Archive] Aggregating finals and backing up to History...");
              ...
              // Log top 3 overall match leaderboard
              const sortedFinals = Array.isArray(finals)
                ? [...finals].sort((a, b) => (b.total || 0) - (a.total || 0))
                : [];
              const podium = ['🥇', '🥈', '🥉'];
              const top3Lines = sortedFinals.slice(0, 3)
                .map((p, i) => `   ${podium[i] || '  '} ${p.name} — ${p.total} pts`)
                .join('\n') || '   (no data)';
              console.log(`
============================================================
🏅 MATCH RESOLVED — Game #${targetMatch.matchNo}: ${targetMatch.home} vs ${targetMatch.away}
   Top 3 Predictions of the Game:
${top3Lines}
   ⏳ Resolving match and archiving...
============================================================`);

              // --- DISCORD: Final Leaderboard ---
              const discordTop3 = sortedFinals.slice(0, 3)
                .map((p, i) => `${['🥇', '🥈', '🥉'][i]} **${p.name}** — ${p.total} pts`)
                .join('\n') || '_No data_';
              await sendDiscordNotification(
                `🏅 **Game #${targetMatch.matchNo} is done! Check the full leaderboard at vrccim.com**`,
                {
                  title: `${targetMatch.home} vs ${targetMatch.away} — Final Standings`,
                  description: `**Top 3 Predictions of the Game:**\n${discordTop3}\n\n` +
                    `🔗 [View Full Leaderboard](https://vrccim.com)`,
                  color: 0xffd600,
                  footer: { text: `Match: ${targetMatch.titleStr}` },
                  timestamp: new Date().toISOString()
                }
